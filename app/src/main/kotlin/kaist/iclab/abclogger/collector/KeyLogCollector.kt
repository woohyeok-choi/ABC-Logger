...
class KeyLogCollector(val context: Context) : AccessibilityService(), BaseCollector {
    ...
    private fun trackNewInput(node: AccessibilityNodeInfo,
                              ...
                              eventTime: Long,
                              isChunjiin: Boolean) {
        if(node.text?.isNotEmpty() == true && "edittext" in node.className.toString().toLowerCase(Locale.ENGLISH)) {
            ...
            ).fillBaseInfo(timeMillis = eventTime).run { putEntity(this) }
    ...
    override val newIntentForSetUp: Intent? = Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)

    companion object {
        const val EXTRA_KEYBOARD_TYPE = "${BuildConfig.APPLICATION_ID}.EXTRA_KEYBOARD_TYPE"
    ...
    class SettingActivity : BaseSettingActivity() {
        ...
        override fun initializeSetting() {
            when(CollectorPrefs.softKeyboardType) {
                KEYBOARD_TYPE_CHUNJIIN -> radio_btn_chunjiin.isChecked = true
                KEYBOARD_TYPE_QWERTY_KOR -> radio_btn_chunjiin.isChecked = true
        ...
        override fun onResume() {
            super.onResume()
            val available =  collector.checkAvailability()
            btn_accessibility_setting.apply {
                setBackgroundColor(
                        ContextCompat.getColor(
                                this@SettingActivity, if(available) R.color.color_primary else R.color.color_accent
                        )
                )
                text = listOf(
                        getString(R.string.general_touch_to_set),
                        getString(if(available) R.string.general_available else R.string.general_unavailable)
        ...
        override suspend fun generateResultIntent(): Intent =
            extraIntentFor(
                    EXTRA_KEYBOARD_TYPE to when {
                        radio_btn_chunjiin.isChecked -> KEYBOARD_TYPE_CHUNJIIN
                        radio_btn_chunjiin.isChecked -> KEYBOARD_TYPE_QWERTY_KOR
                        else -> KEYBOARD_TYPE_UNKNOWN
                    }
            )
    }
}