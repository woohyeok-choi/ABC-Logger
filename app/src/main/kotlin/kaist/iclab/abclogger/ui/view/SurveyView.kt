...
class SurveyView(context: Context, attributeSet: AttributeSet?) : LinearLayout(context, attributeSet) {
    ...
    private val txtDeliveredTime : TextView = TextView(context).apply {
        id = View.generateViewId()
        setTextSize(TypedValue.COMPLEX_UNIT_PX, context.resources.getDimension(R.dimen.txt_size_small_text))
    }
    ...
    fun setEnabledToEdit(isEnabled: Boolean) {
        ...
        txtTitle.setTextColor(ContextCompat.getColor(context, if(isEnabled) R.color.color_title else R.color.color_message))
        ...
        txtDeliveredTime.setTextColor(ContextCompat.getColor(context, if(isEnabled) R.color.color_blue else R.color.color_message))
